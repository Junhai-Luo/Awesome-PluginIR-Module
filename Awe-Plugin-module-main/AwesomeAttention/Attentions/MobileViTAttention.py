...
# PreNorm 类定义了一个标准化和前馈函数的组合
class PreNorm(nn.Module):
    def __init__(self, dim, fn):
        super().__init__()
        self.ln = nn.LayerNorm(dim)  # 层标准化
        self.fn = fn  # 前馈函数

    def forward(self, x, **kwargs):
        return self.fn(self.ln(x), **kwargs)  # 先标准化再通过前馈函数

# FeedForward 类定义了一个前馈网络
...
# Transformer 类定义了一个由多个注意力和前馈网络层组成的 Transformer 结构
...
# MobileViTAttention 类定义了一个结合了局部和全局表示的注意力机制
