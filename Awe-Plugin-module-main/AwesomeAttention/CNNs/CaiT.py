...
class CaiT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 Patch_layer=PatchEmbed, act_layer=nn.GELU, Attention_block=Attention_talking_head, Mlp_block=Mlp,
                 init_scale=1e-4, Attention_block_token_only=Class_Attention, Mlp_block_token_only=Mlp,
                 depth_token_only=2, mlp_ratio_clstk=4.0):
        super().__init__()
        # 定义模型的类别数和特征维度
        self.num_classes = num_classes
        self.num_features = self.embed_dim = embed_dim

        # PatchEmbed层，将图像分割成patches并嵌入
        self.patch_embed = Patch_layer(img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim)

        # 计算patches的数量
        num_patches = self.patch_embed.num_patches

        # 类别token，用于Transformer模型的分类任务
        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim))
        # 位置嵌入，用于编码patches的位置信息
        ...
        # 构建只处理类别token的Transformer block
        self.blocks_token_only = nn.ModuleList([
            block_layers_token(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio_clstk, qkv_bias=qkv_bias,
                               qk_scale=qk_scale,
...
def cait_XXS24(pretrained=False, **kwargs):
    ...
    model = CaiT(
        ...
        mlp_ratio=4,  # MLP比率
        qkv_bias=True,  # 是否在QKV中使用偏置
        norm_layer=partial(nn.LayerNorm, eps=1e-6),  # LayerNorm的epsilon值
        init_scale=1e-5,  # 初始化缩放因子
        depth_token_only=2,  # 仅对token的层数
        **kwargs  # 其他关键字参数
    )

    model.default_cfg = _cfg()  # 设置模型的默认配置
    if pretrained:  # 如果需要预训练权重
        # 从给定的URL加载预训练模型权重
        checkpoint = torch.hub.load_state_dict_from_url(
            url="https://dl.fbaipublicfiles.com/deit/XXS24_384.pth",  # 预训练模型权重的URL
            map_location="cpu",  # 将权重映射到CPU
            check_hash=True  # 检查哈希值是否匹配
        )
        checkpoint_no_module = {}  # 创建一个空字典用于存储去模块化的权重
        for k in model.state_dict().keys():  # 遍历模型权重
            checkpoint_no_module[k] = checkpoint["model"]['module.'+k]  # 去掉模块化前缀
        model.load_state_dict(checkpoint_no_module)  # 加载预训练权重
